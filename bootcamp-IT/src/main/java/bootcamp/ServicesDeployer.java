...
public class ServicesDeployer {
    ...
    public ServicesDeployer(CloudFoundryOperations cf) {
        this.cf = cf;
    }
    ...
    private Flux<Void> unbind(String applicationName, String svcInstanceName, Flux<ServiceInstanceSummary> siFlux) {
        return siFlux.filter(
                si->si.getApplications().contains(applicationName))
                .flatMap(si-> cf.services().unbind(
    ...
    private Publisher<Void> createService(String svcInstanceName, String svcTypeName, String planName) {
